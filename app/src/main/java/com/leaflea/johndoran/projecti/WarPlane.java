...
public class WarPlane extends Vehicles {
    ...
    float g=0;
    public WarPlane(GL10 gl,float MotorX,float MotorY,float MotorZ,float xrot,float yrot,float zrot) {
    ...
    public void generateModel(){
        ...
        float clippedWidth=radius*0.6f;
        ...
        for(int c=0;c<nCylStacks;c++){
            for(int d=0;d<nCylSides;d++) {
                ...
                if(Math.abs(centreCyl[c * nCylSides*3 +d * 3 + 0])>clippedWidth){
                    if(centreCyl[c * nCylSides*3 +d * 3 + 0]<0){
                        centreCyl[c * nCylSides*3 +d * 3 + 0]=-clippedWidth;
                    }else{
                        centreCyl[c * nCylSides*3 +d * 3 + 0]= clippedWidth;
                    }
                }
        ...
        float wingThickness=radius*0.4f;

        propZ=wingZPosition+wingThickness*0.4f+/////past this is patch
                wingThickness*6f;
        propY=0+/////past this is patch
        0.3f;
        propX=wingSpan/2f;
        ...
        for(int c=0;c<wingCylStacks;c++){
            for(int d=0;d<wingCylSides;d++) {
                wingCylVertices[c * wingCylSides*3 +d * 3 + 1] =
                wingCylVertices[c * wingCylSides*3 +d * 3 + 1]*1.1f*(float)(wingCylStacks-c)/(float)(wingCylStacks);

                wingCylVertices[c * wingCylSides*3 +d * 3 + 2] =
                wingCylVertices[c * wingCylSides*3 +d * 3 + 2]*0.5f*(float)(wingCylStacks*3-c*2)/(float)(wingCylStacks);
        ...
        float rudderWidth=1.25f*actualSize;
        float rudderX=0;
...
/*
        ...
        for(int c=0;c<wingCylStacks;c++){
            ...
            for(int d=0;d<wingCylSides;d++) {
                colors[centreCyl.length / 3 * 4 + c *wingCylSides* 4 +d*4+ 0] = R+((float)d/(float)wingCylSides-0.5f)*0.30f;
                colors[centreCyl.length / 3 * 4 + c *wingCylSides* 4 +d*4+ 1] = G+((float)d/(float)wingCylSides-0.5f)*0.25f;
                colors[centreCyl.length / 3 * 4 + c *wingCylSides* 4 +d*4+ 2] = B+((float)d/(float)wingCylSides-0.5f)*0.32f;
                colors[centreCyl.length / 3 * 4 + c *wingCylSides* 4 +d*4+ 3] = 1f;

                colors[(centreCyl.length + wingCylVertices.length) / 3 * 4 + c *wingCylSides* 4 + d*4 + 0] = R+((float)d/(float)wingCylSides-0.5f)*0.30f;
                colors[(centreCyl.length + wingCylVertices.length) / 3 * 4 + c *wingCylSides* 4 + d*4 + 1] = G+((float)d/(float)wingCylSides-0.5f)*0.25f;
                colors[(centreCyl.length + wingCylVertices.length) / 3 * 4 + c *wingCylSides* 4 + d*4 + 2] = B+((float)d/(float)wingCylSides-0.5f)*0.32f;
                colors[(centreCyl.length + wingCylVertices.length) / 3 * 4 + c *wingCylSides* 4 + d*4 + 3] = 1f;
            }
...
*/
    ...
    public void Render(float MotorX,float MotorY,float MotorZ,float xrot,float yrot,float zrot){
        ...
        this.zpos=MotorZ;
        actualSize=calculateSize();
        actualSize=actualSize*mul;
        length  =5f*actualSize;
        width   =3f*actualSize;
        height  =3f*actualSize;



        FloatBuffer vert=rC.vertexBufferGenerator(m.rotorVerts(0.7f * actualSize));
        FloatBuffer color =rC.colorsBufferGenerator(m.rotorColor(255f / 256f, 128f / 256f, 0.25f));
        ShortBuffer ind=rC.indexBufferGenerator(m.rotorIndices());
        g=g+5f;
        gl.glPushMatrix();
        {
            ...
            gl.glPushMatrix();
            {
                ...
                gl.glPushMatrix();
                {
                    gl.glTranslatef(propX * width, propY, propZ * length);

                    gl.glRotatef(g, 0, 0, 1);

                    //rC.drawWithBuffers(gl, true, rC.vertexBufferGenerator(m.rotorVerts(0.7f * actualSize)), rC.colorsBufferGenerator(m.rotorColor(255f / 256f, 128f / 256f, 0.25f)), rC.indexBufferGenerator(m.rotorIndices()), 36);
                    rC.drawWithBuffers(gl,true,vert,color,ind,36);
                }gl.glPopMatrix();
    ...
    public  void fixVerts(){
        ...
        this.propY=(this.propY-difY/2f)*propY;
        this.propZ=(this.propZ-difZ/2f)*propZ;
    }
}
